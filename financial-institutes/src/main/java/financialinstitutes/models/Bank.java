...
//    }
    ...
    public void modifyCredit(BaseEmployee employee, Credit credit, CreditState creditState) throws IncorrectPermissionsException {
        boolean constrainsEmployee = checkConstrainsEmployee(e -> e.permissions.contains(MODIFY_CREDIT), employee);
        boolean constrainsClient = checkConstrainsClient(
                c -> BAD.equals(credit.getClient().getReputation()),
                credit.getClient());

        if (constrainsClient && constrainsEmployee) credit.setState(creditState);
